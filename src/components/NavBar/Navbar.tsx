...
import {
    ...
    useGetBoardsQuery,
    useUpdateBoardMutation, useUpdateBoardNameMutation,
} from "../../api/boardApi";
...
const Navbar = () => {
    ...
    useEffect(() => {
        ...
        if(refetchBoardName){
            refetch().then((data)=>setBoards(data.data));
            setRefetchBoardName(false);
        }
    }, [data, isSuccess, kanbanName,refetchBoardName]);
                            ...
                            boards.map((board: Board) => (
                                ...
                                >
                                    <div className="kanban_element_name_actions">
                                        {fullScreen ? (
                                            <p className="kanban_element_name"
                                               onClick={() => navigate(`/kanban/${board.id!}`)}>{fullScreen ?
                                                board.name : <ViewWeekIcon/> }</p>
                                        ):(
